...
@Directive({
  selector: '[dsMarkdownEditorStickyBottomInset]',
  standalone: true,
})
export class MarkdownEditorStickyBottomInsetDirective implements OnInit, OnDestroy {
  ...
  readonly dsMarkdownEditorStickyBottomInset = input.required<HTMLElement>();
  ...
  ngOnInit(): void {
    ...
    const scope = this.dsMarkdownEditorStickyBottomInset();
