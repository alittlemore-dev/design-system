...
function addCompletedWikiLinkDecorations(
  ...
  if (candidateFrom < 0 || candidateTo > state.doc.length) return;

  const candidate = state.sliceDoc(candidateFrom, candidateTo);
  const references = parseMarkdownWikiLinks(candidate);
  const reference =
    references.length === 1 && references[0]?.raw === candidate ? references[0] : null;
  const data = wikiLinkCompletionData(state);
  if (
    reference === null ||
    data === null ||
    !data.namespaces.some((namespace) => namespace.key === reference.namespace)
  ) {
    return;
  }

  const namespaceFrom = candidateFrom + 2;
  const colonFrom = namespaceFrom + reference.namespace.length;
  const keyFrom = colonFrom + 1;
  const keyTo = keyFrom + reference.key.length;
  addWikiLinkDecoration(
    { from: candidateFrom, to: namespaceFrom },
    visibleRange,
  ...
  addWikiLinkDecoration(
    { from: namespaceFrom, to: colonFrom },
    visibleRange,
    'cm-wiki-link-namespace',
    ranges,
    rangeKeys,
  );
  addWikiLinkDecoration(
    { from: colonFrom, to: keyFrom },
    visibleRange,
    'cm-wiki-link-colon',
    ranges,
    rangeKeys,
  ...
  const separator = state.sliceDoc(keyTo, keyTo + 2);
  const separatorLength = separator.startsWith('\\|') ? 2 : separator.startsWith('|') ? 1 : 0;
  if (separatorLength > 0) {
    addWikiLinkDecoration(
      { from: keyTo, to: keyTo + separatorLength },
      visibleRange,
    ...
    addWikiLinkDecoration(
      { from: keyTo + separatorLength, to: candidateTo - 2 },
      visibleRange,
      'cm-wiki-link-label',
      ranges,
      rangeKeys,
...
function addActiveWikiLinkDecorations(
  ...
  if (
    ...
    !data.namespaces.some((namespace) => namespace.key === active.targetType)
  ) {
    return;
  }
  ...
  for (const visibleRange of visibleRanges) {
    ...
    addActiveWikiLinkRange(active.domain, visibleRange, 'namespace', ranges, rangeKeys);
    if (active.colon !== null) {
      addActiveWikiLinkRange(active.colon, visibleRange, 'colon', ranges, rangeKeys);
    }
    if (active.slug !== null) {
      addActiveWikiLinkRange(active.slug, visibleRange, 'key', ranges, rangeKeys);
    }
  }
}
...
function addActiveWikiLinkRange(
  ...
  visibleRange: MarkdownVisibleRange,
  semanticClass: 'delimiter' | 'namespace' | 'colon' | 'key',
  ranges: Range<Decoration>[],
  rangeKeys: Set<string>,
): void {
  addWikiLinkDecoration(
...
function addWikiLinkDecoration(
  ...
  if (from >= to || rangeKeys.has(key)) return;
  rangeKeys.add(key);
