...
export class SiteSelectComponent implements ControlValueAccessor, OnChanges, Validator {
  ...
  /** @internal */
  protected readonly listboxTestId = computed(() => {
  ...
  /** @internal */
  protected readonly committedValue = computed(() => this.value() ?? this.formValue());

  /** @internal */
  protected readonly selectedOption = computed(
    () => this.options().find((option) => option.value === this.committedValue()) ?? null,
  );
  /** @internal */
  protected readonly displayLabel = computed(() => this.selectedOption()?.label ?? '');
  ...
  /** @internal */
  protected readonly renderedOptions = computed<readonly RenderedSiteSelectOption[]>(() => {
    const activeIndex = this.activeIndex();
    const committedValue = this.committedValue();
  ...
  private readonly openStateSyncEffect = effect(() => {
    const options = this.options();
    const committedValue = this.committedValue();
    if (!untracked(() => this.open())) return;
    if (this.effectiveDisabled() || options.length === 0) {
      this.closeList(false);
      return;
    }
    const selectedIndex = options.findIndex((option) => option.value === committedValue);
    this.activeIndex.set(selectedIndex < 0 ? 0 : selectedIndex);
  });
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['required'] !== undefined) this.onValidatorChange?.();
    if (changes['controlDisabled'] !== undefined && this.controlDisabled() && this.open()) {
      this.closeList(false);
    }
  }
  ...
  /** @internal */
  protected chooseOption(index: number): void {
