...
- [ ] Create and verify the `alittlemore.dev` npm organization, bootstrap `@alittlemore.dev/design-system@0.2.0` through CI, configure trusted publishing, revoke the scope-migration token and delete `NPM_TOKEN`, then verify the next ordinary OIDC release.
